...
def pascal_triangle(n):
    ...
    if (n <= 0):
        ...
        # from 0 to the desired number of rows (n).
        for row in range(n):
            # For each row, initialize a list (row)
            # with 1 repeated i + 1 times
            # where i is the current row index.
            row_list = [1] * (row + 1)

            # Use another loop to calculate the values
            # inside the row.
            # Start the loop from the second element
            # (j = 1) up to the second-to-last element (i - 1).
            for v in range(1, row):
                # Set row[j] to the sum of the values
                # from the previous
